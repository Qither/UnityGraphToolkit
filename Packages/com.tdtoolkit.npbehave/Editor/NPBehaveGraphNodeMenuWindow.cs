...
{
    class NPBehaveGraphNodeMenuWindow : ScriptableObject, ISearchWindowProvider
    {
        ...
        void CreateStandardNodeMenu(List<SearchTreeEntry> tree)
        {
            ...
            var nodeEntries = graphView.FilterCreateNodeMenuEntries().OrderBy(k => k.order).ThenBy(k => k.path);
            ...
            foreach (var nodeMenuItem in nodeEntries)
            {
                ...
                tree.Add(new SearchTreeEntry(new GUIContent(nodeName, icon)) { level = level + 1, userData = nodeMenuItem.type });
            }
        }
