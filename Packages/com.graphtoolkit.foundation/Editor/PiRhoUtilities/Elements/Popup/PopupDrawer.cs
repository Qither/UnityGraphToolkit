...
namespace PiRhoSoft.Utilities.Editor
{
	...
	class PopupDrawer : PropertyDrawer
	{
		...
		private VisualElement CreatePopup<T>(SerializedProperty property, PopupAttribute popupAttribute, List<T> defaultValues)
		{
            ...
            if (string.IsNullOrEmpty(popupAttribute.ValuesSource) && null != popupAttribute.Options)
            {
                var defaultOptions = new PopupValues<T> { Values = defaultValues, Options = popupAttribute.Options };
                setValuesWithOptions(defaultOptions);
            }
			else if (!ReflectionHelper.SetupValueSourceCallback(popupAttribute.ValuesSource, fieldInfo.DeclaringType, property, popup, defaultValues, popupAttribute.AutoUpdate, setValues))
			{
				var defaultOptions = new PopupValues<T> { Values = defaultValues, Options = popupAttribute.Options };
