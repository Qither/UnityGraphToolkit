...
namespace GraphProcessor
{
	public static class NodeProvider
	{
		...
		public class NodeDescriptions
		{
			...
			public Dictionary< string, int >		nodeMenuOrder = new Dictionary< string, int >();
		...
		static void BuildCacheForNode(Type nodeType, NodeDescriptions targetDescription, BaseGraph graph = null)
		{
			...
			if (attrs != null && attrs.Length > 0)
			{
				foreach (var attr in attrs)
				{
					...
					targetDescription.nodeMenuOrder[attr.menuTitle] = attr.order;
				}
			}
        ...
        public static IEnumerable<(string path, Type type, int order)>	GetNodeMenuEntries(BaseGraph graph = null)
		{
			foreach (var node in genericNodes.nodePerMenuTitle)
			{
				int order = genericNodes.nodeMenuOrder.TryGetValue(node.Key, out var o) ? o : 0;
				yield return (node.Key, node.Value, order);
			}
			...
			if (graph != null && specificNodeDescriptions.TryGetValue(graph, out var specificNodes))
			{
				...
				{
					int order = specificNodes.nodeMenuOrder.TryGetValue(node.Key, out var o) ? o : 0;
					yield return (node.Key, node.Value, order);
				}
			}
		}
