...
namespace GraphProcessor
{
    ...
    class CreateNodeMenuWindow : ScriptableObject, ISearchWindowProvider
    {
        ...
        void CreateStandardNodeMenu(List<SearchTreeEntry> tree)
        {
            ...
            var nodeEntries = graphView.FilterCreateNodeMenuEntries().OrderBy(k => k.Item3).ThenBy(k => k.Item1);
            var titlePaths = new HashSet< string >();
            
			foreach (var nodeMenuItem in nodeEntries)
			{
                var nodePath = nodeMenuItem.Item1;
                ...
                if(parts.Length > 1)
                {
                    ...
                    for(var i = 0; i < parts.Length - 1; i++)
                    {
                        ...
                        {
                            tree.Add(new SearchTreeGroupEntry(new GUIContent(title, icon)){
                                level = level
                            });
