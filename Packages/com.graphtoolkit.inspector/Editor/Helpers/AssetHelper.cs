...
namespace GraphToolkit.Inspector.Editor
{
	...
	public class AssetHelper : AssetPostprocessor
	{
		...
		private static string _embeddedPackageRoot = "/Packages/";
		...
		public static string GetAssetPath(string fullPath)
		{
			...
			var embeddedPackageIndex = normalized.LastIndexOf(_embeddedPackageRoot, StringComparison.Ordinal);
			if (embeddedPackageIndex >= 0)
			{
				var relativePackageIndex = embeddedPackageIndex + 1;
				var directoryEndIndex = normalized.LastIndexOf('/');
				if (directoryEndIndex >= relativePackageIndex)
					return normalized.Substring(relativePackageIndex, directoryEndIndex - relativePackageIndex + 1);
			}
